...
import time
from src.common.config import Config,SCREEN_PATH
import pywinauto
from pywinauto.keyboard import send_keys


logger = Logger(logger_name='Verify').get_logger()
...
class Action(object):
    ...
    def is_element_present(self, loc, timeout=10):
        """
        判断控件元素是否存在
        """
        try:
            ...
            logger.error("元素没定位到：" + str(loc))
            raise
        else:
            return result

    def file_uploads(self,filepath,filename):
        """
        web上传文件
        :param filepath:
        :param filename:
